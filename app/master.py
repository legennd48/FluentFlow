...
class FluentFlow:
  ...
  def translate(self, text_or_image, target_language, is_image=False):
    ...
    Args:
      ...
      is_image: Flag indicating whether the input is an image (True) or text (False).
    ...
    cached_translation = self.cache.check_cache(source_text=extracted_text, target_language=target_language)

    if not cached_translation:
      translated_text = self.translator.translate(text=extracted_text, target_language=target_language)
      if translated_text:
         # self.cache.update_cache(source_language='en',  source_text=extracted_text, target_language, translated_text
        self.cache.save_translation(source_language=source_language, source_text=extracted_text, target_language=target_language, translated_text=translated_text)
    else:
...
if __name__ == "__main__":
    ...
    target_language = "yo"
    translated_text = fluent_flow.translate(image_path, target_language, is_image=True)
    ...
    print(f"Translated text (direct input): {translated_text}")
